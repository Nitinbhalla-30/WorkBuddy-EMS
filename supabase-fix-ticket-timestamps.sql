-- Fix: query/grievance notification timestamps show 12:00 AM instead of the real time
--
-- The app writes full ISO datetimes for created_on, updated_on and each
-- message's "on" stamp, but created_on/updated_on were created as plain DATE,
-- so Postgres truncated the time on every upsert and the notification list
-- showed midnight. (Same issue and fix as supabase-fix-leave-timestamps.sql.)
--
...
-- 12:00 AM. New submissions and replies store the real time.

-- NOTE: This migration was applied to the Supabase project on 2026-09-02 via
-- the Supabase MCP (saved as migration "fix_ticket_timestamp_columns").
