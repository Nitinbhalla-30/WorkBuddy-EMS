...
type OriginButtonProps = ButtonHTMLAttributesForMotion & {
  ...
  loading?: boolean;
  /** Extra classes for the expanding fill circle (defaults to foreground color). */
    ...
    return (
      <motion.button
        ...
        className={cn(
          ...
          // Resting surface matches the topbar's other controls (e.g. the
          // notification bell): same border, background and ink color.
          "border border-[var(--line)] bg-[var(--btn-light-bg)] text-[var(--ink)]",
      ...
      >
        <motion.span
          ...
          aria-hidden
          className={cn(
            "pointer-events-none absolute rounded-full bg-foreground dark:bg-neutral-50",
            fillClassName
          )}
          initial={false}
          style={{
            ...
            top: origin.y,
            width: coverSize,
            // Center the circle on the pointer origin via motion-owned
            // transforms — Tailwind translate utilities would be
            // overwritten by framer-motion's scale animation.
            x: "-50%",
            y: "-50%",
          }}
