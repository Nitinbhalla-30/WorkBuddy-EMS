...
The app shell is a fixed-viewport flex layout: 60px topbar + 247px sidebar + fluid content area. The sidebar can collapse to a 64px icon rail from a "Hide menu" row pinned to the bottom of the panel — icons, active state and unread badges survive the collapse, only the labels give way. The sidebar collapses to a horizontal scroll bar on mobile (720px breakpoint), where rail mode does not apply. Content padding is 26px on desktop, reducing naturally on smaller viewports.
...
- **Decisions ask first, and one per pop-up:** irreversible row actions (approve, reject, cancel) reached from the three-dot menu open a confirm modal before writing — clicking the menu item is intent, not consent. Each decision pop-up carries only that decision: the reject dialog shows the reason field alone, the question dialog shows the thread alone; Approve/Reject buttons are never repeated inside them. The confirm copy states what will change and that the decision is final, and the primary button is disabled and relabelled while the write is in flight ("Approving…"). Cancel always means back out, never a half-finished state.
...
- **Variants:** ok (green), late (amber), absent (gray — also the withdrawn pill), pending (blue), closed (slate), high (red), medium (amber), low (teal), pickup (amber), drop (blue), event (green), general (gray).
- **Terminal statuses:** In the ticket and IT-issue ladders, Closed uses `tag-closed` (deep slate, ink text) and Withdrawn uses `tag-absent` (pale grey, muted text). Never red — red means someone still has to act — and never the same pill, because a closed record was completed while a withdrawn one was voided by the person who raised it.
- **Derived statuses:** a row may display a state that is not stored. An IT issue the employee re-opened reads "Reopened" (`tag-it-reopened`, violet — the one hue the IT table had not used; amber/blue/green/slate/grey are taken and red belongs to High priority in the neighbouring column) while its stage stays `open`. Always go through `itIssueDisplayStatus(issue)` (src/utils/itIssues.js) so the Status column, the sort, the search box and the status filter all read the row the same way, and share `IT_ISSUE_STATUS_FILTER_OPTS` rather than rebuilding the list per page. Such a value never joins `IT_ISSUE_STATUSES`, which drives the picker that *sets* a stage — where it does appear as a current value, render it as a `disabled` option so it can be read but not chosen.
- **State:** Static classification — not interactive. Always paired with a colored background + matching text color.

### Navigation
- **Style:** 247px sidebar with 8px-radius items. 13.5px font, 500 weight default, 600 weight active.
...
- **Nav scroll:** vertical only on desktop — `overflow-x` is pinned to `hidden` so expanding the rail can't flash a horizontal scrollbar while the labels widen (an `overflow-y: auto` container would otherwise grow one by default). The vertical bar stays because short screens need it; it is hidden in rail mode, where nothing overflows sideways.
- **Rail badges:** The unread count becomes a 16px pill anchored to the row's top-right corner, lifted onto the row's top edge so it never sits over the glyph — a two-digit pill is too wide to clear the 17px icon horizontally inside a 64px rail, so the clearance is vertical.
